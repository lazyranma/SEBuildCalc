...
function updateBonusNotes(locale) {{
  document.querySelectorAll('.tech-note[data-targets]').forEach(el => {{
    ...
    const names = targets.map(target => {{
      if (target === 'All') return t('bonus_target_all', locale);
      if (target === 'Facility') return t('bonus_target_facility', locale);
      if (target === 'LV') return t('bonus_target_lv', locale);
      if (target === 'SC') return t('bonus_target_sc', locale);
      if (target === 'SComponent' || target === 'RD') return null;
      // Specific item ID — use game loc and uppercase
      const name = getGameLocName(target, locale);
...
function applyLocale(locale) {{
  // Save preference first, before any DOM updates that might fail
